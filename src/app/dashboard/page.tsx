...
export default function BusinessOwnerDashboard() {
  ...
  const handleSendChat = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/business/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: userText, customerPhone: "081234567890" }),
      });
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EE] text-[#1F2A24] p-4 sm:p-6 md:p-10 w-full overflow-x-hidden">
      {/* Header Dashboard UMKM (Fully Responsive) */}
      ...
      <main className="max-w-7xl mx-auto space-y-6">
        ...
        {activeTab === "laporan" && (
          <div className="space-y-6">
            ...
            {/* Kartu Ringkasan Laporan (Responsive 1/2/4 Cols) */}
            ...
            {/* Tabel Ringkas Produk Stok Menipis */}
        ...
        {/* TAB 6: SIMULASI CHAT AI */}
        {activeTab === "chat" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
              <Card className="p-0 overflow-hidden border-[#E2E0D8] shadow-sm">
                <div className="bg-[#2F6A55] text-white p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/20 text-white flex items-center justify-center font-bold text-base sm:text-lg">
                      🤖
                    </div>
                    <div>
                      <h3 className="font-bold text-sm leading-tight">{tenant?.name} — Admin AI</h3>
                      <span className="text-[11px] text-emerald-100 flex items-center gap-1 mt-0.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-300 animate-pulse"></span>
                        Simulasi Chat WA (Customer: 081234567890)
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    {usingApiKey === true && (
                      <span className="px-2.5 py-1 bg-emerald-700/60 text-emerald-100 text-[10px] font-semibold rounded-md border border-emerald-400/30 block sm:inline-block">
                        ⚡ Google Gemini Tool Use Live
                      </span>
                    )}
                    {usingApiKey === false && (
                      <span className="px-2.5 py-1 bg-amber-600/60 text-amber-100 text-[10px] font-semibold rounded-md border border-amber-400/30 block sm:inline-block">
                        💡 Mode Tool Use Cerdas (No API Key)
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div
                  ref={chatScrollRef}
                  className="h-[380px] sm:h-[430px] bg-[#EFEAE2] p-3 sm:p-4 overflow-y-auto space-y-3 text-xs font-normal"
                >
                  {conversations.length === 0 ? (
                    <div className="text-center py-16 space-y-2 text-[#6B7570]">
                      <div className="w-12 h-12 rounded-full bg-white/80 flex items-center justify-center mx-auto text-xl shadow-xs">
                        💬
                      </div>
                      <p className="font-bold text-sm text-[#1F2A24]">Belum Ada Riwayat Simulasi Chat</p>
                      <p className="text-xs max-w-xs mx-auto">
                        Ketik pesan seperti &quot;Jual apa aja kak?&quot; atau &quot;Saya mau pesan 2 pack Frozen Beef&quot; di bawah ini.
                      </p>
                    </div>
                  ) : (
                    conversations.map((chat) => (
                      ...
                      >
                        <div
                          className={`max-w-[88%] sm:max-w-[80%] rounded-2xl px-3.5 py-2.5 shadow-xs whitespace-pre-wrap leading-relaxed ${
                        ...
                        >
                          <div className="text-[10px] opacity-70 mb-0.5 font-semibold">
                            {chat.sender === "PELANGGAN" ? "Pelanggan (081234567890)" : `Admin AI (${tenant?.name})`}
                          </div>
                  ...
                  {sendingChat && (
                    <div className="flex justify-end">
                      <div className="bg-[#2F6A55]/80 text-white rounded-2xl rounded-tr-none px-3.5 py-2 text-xs flex items-center gap-2 shadow-xs">
                        ...
                        <span className="text-[10px]">AI sedang mengeksekusi Tool & membalas...</span>
                      </div>
                    </div>
                  )}
                </div>

                <form onSubmit={handleSendChat} className="bg-white p-2.5 sm:p-3 border-t border-[#E2E0D8] flex gap-2 items-center">
                  <input
                    type="text"
                    required
                    placeholder="Tulis pesan simulasi... (cth: jual apa aja kak?)"
                    value={chatInput}
                  ...
                  />
                  <Button variant="primary" type="submit" className="py-2.5 px-4 text-xs whitespace-nowrap" disabled={sendingChat}>
            ...
            <div className="space-y-4 text-xs">
              <Card title="Skenario Pengujian AI Chat">
                <div className="space-y-3 text-xs leading-relaxed text-[#6B7570]">
                  <div className="p-2.5 bg-[#2F6A55]/10 text-[#2F6A55] rounded-xl border border-[#2F6A55]/20 font-medium">
                    🛍️ <strong className="block text-[#1F2A24]">1. Tes Tanya Katalog & Harga:</strong>
                    Ketik: <em>&quot;Jual apa aja kak?&quot;</em> atau <em>&quot;Daftar harga&quot;</em>
                  </div>

                  <div className="p-2.5 bg-[#2F6A55]/10 text-[#2F6A55] rounded-xl border border-[#2F6A55]/20 font-medium">
                    🎯 <strong className="block text-[#1F2A24]">2. Tes Tool &quot;catat_pesanan&quot;:</strong>
                    Ketik: <em>&quot;Saya mau pesan 2 pack Frozen Beef Teriyaki&quot;</em>
                  </div>

                  <div className="p-2.5 bg-[#B8483F]/10 text-[#B8483F] rounded-xl border border-[#B8483F]/20 font-medium">
                    🚨 <strong className="block text-[#1F2A24]">3. Tes Tool &quot;alihkan_ke_admin&quot;:</strong>
                    Ketik: <em>&quot;Saya sudah transfer Rp 90.000 ke BCA ya kak&quot;</em>
                  </div>
                </div>
              </Card>
            </div>
